...
import { fetchTransportListings } from '../api/transport'
...
function BookingsTab({ t }) {
  ...
  // Use live jobs if available, or fallback to static
  const jobs = !loadingJobs && liveJobs.length > 0 ? liveJobs : JOBS

  const accept = (id) => setAccepted(a => ({ ...a, [id]: true }))
  const decline = (id) => {/* remove from live list – for static data just skip */ }
    ...
    <div>
      ...
      <div className={s.grid2}>
        ...
        <div>
          ...
          {loadingJobs && <div style={{ padding: '1rem', color: 'var(--text-soft)' }}>Loading jobs…</div>}
          {jobs.map((j, idx) => {
            const isLive = !!j.route_from
            const jobId = isLive ? j.id : j.id
            const from = isLive ? j.route_from : j.from
            const to = isLive ? j.route_to : j.to
            const pay = isLive ? `₹${j.price?.toLocaleString('en-IN')}` : j.pay
            const cap = isLive ? j.capacity : j.km
            const notes = isLive ? (j.notes || j.vehicle_type) : j.crop
            const key = jobId || idx
            return (
              <div key={key} className={`${s.jobCard} ${s.jobNew}`}>
                <div className={s.jobHeader}>
                  <div className={s.jobId}>{isLive ? `#${String(jobId).slice(-6).toUpperCase()}` : jobId}</div>
                ...
                <div className={s.jobDetails}>
                  {[notes, pay, cap].filter(Boolean).map((d, di) => (
                    <span key={di} className={s.jobDetail}>{d}</span>
                  ))}
                </div>
                <div className={s.jobActions}>
                  {accepted[key] ? (
                    ...
                    <>
                      <button className={s.btnAccept} onClick={() => accept(key)}>{t('accept_btn')}</button>
