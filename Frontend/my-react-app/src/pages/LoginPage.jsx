...
function pwScore(v) {
  ...
  return { n, color:['','#e74c3c','#e67e22','#f1c40f','#2ecc71','#27ae60'][n]||'',
...
export default function LoginPage() {
  ...
  // login
  const [lPhone, setLPhone] = useState(''); const [lPw, setLPw] = useState(''); const [showLPw, setShowLPw] = useState(false)
  // register
  const [rFirst,setRFirst]=useState(''); const [rLast,setRLast]=useState('')
  const [rPhone,setRPhone]=useState(''); const [rDist,setRDist]=useState(''); const [rState,setRState]=useState('')
  ...
  const pw = tab==='login'?lPw:rPw; const str = pwScore(pw)

  const go = () => {
    const u = tab==='login'
      ? { name:'Ramesh Patil', role }
      : { name:`${rFirst} ${rLast}`, role, vehicle:vNum||undefined, vehType:vType||undefined }
    login(u); setSuccess(true)
  }

  const doLogin = () => {
    setErr('')
    if (!lPhone||lPhone.length<10) { setErr('Please enter a valid 10-digit mobile number.'); return }
    if (!lPw) { setErr('Please enter your password.'); return }
    setLoading(true); setTimeout(()=>{ setLoading(false); go() }, 1300)
  }
    ...
    <div className={s.root}>
      <div className={s.card} style={{maxWidth:420,margin:'auto'}}>
        <div className={s.successWrap}>
          <span className={s.successIcon}>{role==='driver'?'🚛':'👨‍🌾'}</span>
          <h2 className={s.successTitle}>{tab==='login'?t('login_success'):t('account_created')}</h2>
          <p className={s.successSub}>{t('welcome_back')}, {tab==='register'?rFirst:'Ramesh'}! {t('redirecting')}</p>
          <div className={s.rolePill}>{role==='driver'?t('driver_account'):t('farmer_account')}</div>
          <button className={s.btnPrimary} onClick={()=>nav(role==='driver'?'/driver':'/farmer')}>{t('go_dashboard')}</button>
        </div>
      </div>
    </div>
  )
          ...
          <div className={s.body}>
            {otp ? (
              ...
              <div>
                ...
                <button className={s.btnPrimary} style={{marginTop:20}} onClick={()=>go()}>{t('verify_login')}</button>
              </div>
            ) : (
              <>
                {/* Role cards */}
                ...
                {tab==='login' ? (
                  ...
                  <>
                    ...
                    <div className={s.forgotRow}>
                      <button className={s.forgotBtn} onClick={()=>lPhone.length>=10&&alert(`Reset OTP sent to +91 ${lPhone}`)}>
                        {t('forgot_pw')}
                      </button>
                    </div>
                    ...
                    <div className={s.orDiv}><span>{t('or_with')}</span></div>
                    <button className={s.btnOtp} onClick={()=>setOtp(true)}>{t('login_otp')}</button>
                  </>
                ) : (
                  /* ── Register form ── */
                  <>
